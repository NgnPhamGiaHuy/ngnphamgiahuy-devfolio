...
const TestimonialCard: React.FC<TestimonialCardProps> = ({ item }) => {
    ...
    return (
        <div className={"w-full h-auto flex-center shrink-0 transition-transform transform translate-z-0 backface-hidden box-border relative"}>
            <div className={"w-full py-[30px] px-[35px] bg-card-inverse rounded-[18px] overflow-hidden relative"}>
                <div className={"w-full mb-[30px] block relative"}>
                    <Image src={imageUrl} alt={imageAlt} width={600} height={400} className={"max-w-full w-full h-[180px] rounded-[18px] object-center object-cover border-none shadow-none relative"} />
                ...
                <div className={"mt-[30px]"}>
                    <h6 className={"m-0! text-[24px] max-md:text-[18px] text-inverse!"}>
                        <span>{item.name}</span>
                    </h6>
                    <div>
