...
export function useDynamicTextAnimation() {
    ...
    const createTypingTimeline = (textSections: Array<{id: string, text: string}>) => {
        let currentDelay = 0;
        const timeline: Record<string, { sectionDelay: number, charDelays: number[] }> = {};
        ...
        textSections.forEach((section) => {
            ...
            const nextSectionDelay = Math.max(MIN_SECTION_DURATION, typingDuration) + PAUSE_AFTER_SECTION;
